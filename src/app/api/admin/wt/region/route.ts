...
export async function GET(req: NextRequest) {
	...
	try {
		...
		return NextResponse.json(
			{ error: "Failed to fetch the selected Region" },
			{ status: 500 }
		);
	}
}
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		if (!name || !contact) {
			return NextResponse.json(
				{ error: "Bad Request. regionName and regionContact must be provided" },
				{ status: 400 }
			);
		}
		...
		if (!id) {
			return NextResponse.json(
				{ error: "Invalid or missing region ID" },
				{ status: 400 }
			);
		}
		...
		if (!updatedRegion) {
			return NextResponse.json(
				{ error: "Region not found or update failed" },
				{ status: 404 }
			);
		}
